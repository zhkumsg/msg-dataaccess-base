...
var Single = {
  ...
  querySQL(sql, condition, size, pagesize, pageno, isPager, sp, flag) {
    ...
    let promise = new Promise((resolve, reject) => {
      if (isPager) {
        ds.GetTable("select count(1) as sam from (" + sql + ") as myTemp").then(
          (dt, err) => {
            if (err) {
              reject(err);
            } else {
              ...
              ds.GetTable(ds.datapagerSql(pageno, pagesize, sql, sp)).then(
                (dt, err) => {
                  if (err) {
                    reject(err);
                  } else {
  ...
  getwhere(condition, flag) {
    ...
    condition.forEach(mc => {
      ...
      switch (mc.Type) {
        case MType.Mstring:
          {
            if (!mc.value && mc.Operator !== MOperator.IsNull) {
              return false;
            }
        ...
        case MType.Mint:
          {
            if (mc.value == null && mc.Operator != MOperator.IsNull) {
              return false;
            }
        ...
        case MType.Mdatetime:
          {
            ...
            if (mc.Logic == MLogic.And) {
              ...
              switch (mc.Operator) {
                ...
                case MOperator.LargeThan:
                  sqlwhere +=
                    " or " + fieldstr + ">" + ds.parse(new Date(mc.value)) + "";
                  break;
                case MOperator.LargeEqualThan:
                  sqlwhere +=
                    " or " +
  ...
  DeleteByIds(sql, ids) {
    if (ids.length === 0) {
      return Promise.reject("缺少条件，无法删除");
    }
    let idsstr = [];
