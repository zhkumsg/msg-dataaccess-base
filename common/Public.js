...
var Public = {
  ...
  OperationSQLParams(obj, operationType) {
    ...
    switch (operationType) {
      case OperationEnum.Create:
        return this.InsertSQLParams(obj, tableName);
        break;
      case OperationEnum.Delete:
        return this.DeleteSQLParams(obj, tableName, pkNames);
        break;
      case OperationEnum.Update:
        return this.UpdateSQLParams(obj, tableName, pkNames);
        break;
      case OperationEnum.UpdateNoCheck:
        return this.UpdateNoCheckSQLParams(obj, tableName, pkNames);
        break;
      default:
        return null;
        break;
      ...
      if (typeof obj === "object" && tableName != undefined) {
        ...
        if (properties.length > 0) {
          ...
          properties.forEach(pf => {
            ...
            if (
              ["EB_CREATE_DATETIME", "EB_LASTMODIFY_DATETIME"].indexOf(
                pf.toUpperCase()
              ) > -1
            ) {
              params.push(
        ...
        if (properties.length > 0) {
          ...
          properties.forEach(pf => {
            ...
            if (pkNames.indexOf(pf) > -1) {
              ...
              ["EB_CREATE_DATETIME", "EB_CREATEBY"].indexOf(
                pf.toUpperCase()
              ) === -1
            ) {
  ...
  StringToDate(datestr) {
    ...
    if (reg.test(datestr)) {
      ...
      if (
        /^\d{4}-\d{1,2}-\d{1,2}T\d{1,2}:\d{1,2}:\d{1,2}.\d+Z$/gi.test(datestr)
      ) {
